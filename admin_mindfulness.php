...
if(isset($_POST['add_library_mindfulness'])){
   $l_name = $_POST['l_name'];
   ...
   $insert_query = mysqli_query($connect, "INSERT INTO `library_mindfulness`(name, video, description) VALUES('$l_name','$l_video', '$l_description')") or die('query failed');
...
if(isset($_GET['delete'])){
   ...
   $delete_query = mysqli_query($connect, "DELETE FROM `library_mindfulness` WHERE id = $delete_id ") or die('query failed');
   if($delete_query){
      header('location:admin_mindfulness.php');
...
if(isset($_POST['update_library_mindfulness'])){
   ...
   $update_query = mysqli_query($connect, "UPDATE `library_mindfulness` SET name = '$update_l_name', video = '$update_l_video', description = '$update_l_description' WHERE id = '$update_l_id'");
...
<head>
   ...
   <title>Admin Mindfulness Panel</title>
...
<img src="images/kk.png" width="60" height="60" alt="meditation"/>
      ...
      <a href="http://localhost/meditation/admin_guidedmeditation.php">Guided </a>
                    ...
                    <a href="http://localhost/meditation/library_mindfulness.php">LIBRARY</a>
	  ...
	  <h1 class="heading">MINDFULNESS</h1>
...
<form action="" method="post" class="add-library-form" enctype="multipart/form-data">
   ...
   <input type="submit" value="add the library" name="add_library_mindfulness" class="btn">
</form>
...
<section class="display-product-table">
   ...
   if(mysqli_num_rows($select_library_mindfulness) > 0){
      while($row = mysqli_fetch_assoc($select_library_mindfulness)){
   ?>
   ...
   <tr>
      ...
      <td>
         <a href="admin_mindfulness.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('Are you sure you want to delete this?');">
            <i class="fas fa-trash"></i> Delete
         </a>
         <a href="admin_mindfulness.php?edit=<?php echo $row['id']; ?>" class="option-btn">
...
<section class="edit-form-container">
   ...
   if(isset($_GET['edit'])){
      $edit_id = $_GET['edit'];
      $edit_query = mysqli_query($connect, "SELECT * FROM `library_mindfulness` WHERE id = $edit_id");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="submit" value="update the library" name="update_library_mindfulness" class="btn">
