...
if (isset($_POST['add_to_bookmark'])) {
    $library_sleep_name = $_POST['library_sleep_name'];
    $library_sleep_video = $_POST['library_sleep_video'];
    $library_sleep_description = $_POST['library_sleep_description'];
    $user_name = $_SESSION['user_name']; // Get the user's ID from the session

    // Check if the library is already in the user's bookmarks
    $check_existing_query = "SELECT * FROM `bookmark` WHERE name = '$library_sleep_name' AND video = '$library_sleep_video' AND user_name = '$user_name'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $insert_library_sleep = "INSERT INTO `bookmark`(name, video, description, user_name) 
                         VALUES('$library_sleep_name', '$library_sleep_video', '$library_sleep_description', '$user_name')";
        mysqli_query($connect, $insert_library_sleep);
                ...
                while ($fetch_library = mysqli_fetch_assoc($select_library_sleep)) {
                    ...
                    <form action="" method="post">
                        <div class="box">
                            ...
                            <input type="hidden" name="library_sleep_name" value="<?php echo $fetch_library['name']; ?>">
                            <input type="hidden" name="library_sleep_video" value="<?php echo $fetch_library['video']; ?>">
                            <input type="hidden" name="library_sleep_description"
