...
if (isset($_POST['add_to_bookmark'])) {
    $library_yoga_name = $_POST['library_yoga_name'];
    $library_yoga_video = $_POST['library_yoga_video'];
    $library_yoga_description = $_POST['library_yoga_description'];
    $user_name = $_SESSION['user_name']; // Get the user's ID from the session

    // Check if the library is already in the user's bookmarks
    $check_existing_query = "SELECT * FROM `bookmark` WHERE name = '$library_yoga_name' AND video = '$library_yoga_video' AND user_name = '$user_name'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $insert_library_yoga = "INSERT INTO `bookmark`(name, video, description, user_name) 
                         VALUES('$library_yoga_name', '$library_yoga_video', '$library_yoga_description', '$user_name')";
        mysqli_query($connect, $insert_library_yoga);
                ...
                while ($fetch_library = mysqli_fetch_assoc($select_library_yoga)) {
                    ...
                    <form action="" method="post">
                        <div class="box">
                            ...
                            <input type="hidden" name="library_yoga_name" value="<?php echo $fetch_library['name']; ?>">
                            <input type="hidden" name="library_yoga_video" value="<?php echo $fetch_library['video']; ?>">
                            <input type="hidden" name="library_yoga_description"
