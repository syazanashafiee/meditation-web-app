...
if(isset($_POST['add_library_quotes'])){
   $l_name = $_POST['l_name'];
   ...
   $insert_query = mysqli_query($connect, "INSERT INTO `library_quotes`(name, image, description) VALUES('$l_name','$l_image', '$l_description')") or die('query failed');
...
if(isset($_GET['delete'])){
   ...
   $delete_query = mysqli_query($connect, "DELETE FROM `library_quotes` WHERE id = $delete_id ") or die('query failed');
   if($delete_query){
...
if(isset($_POST['update_library-quotes'])){
   ...
   $update_query = mysqli_query($connect, "UPDATE `library_quotes` SET name = '$update_l_name', image = '$update_l_image', description = '$update_l_description' WHERE id = '$update_l_id'");
...
<img src="images/kk.png" width="60" height="60" alt="meditation"/>
      ...
      <a href="http://localhost/meditation/admin_guidedmeditation.php">Guided </a>
                    ...
                    <a href="http://localhost/meditation/library_quotes.php">LIBRARY</a>
...
<form action="" method="post" class="add-library-form" enctype="multipart/form-data">
   ...
   <input type="submit" value="add the library" name="add_library_quotes" class="btn">
</form>
      ...
      <tbody>
         <?php
            ...
            if(mysqli_num_rows($select_library_quotes) > 0){
               while($row = mysqli_fetch_assoc($select_library_quotes)){
         ?>

         <tr>
            <td><img src="uploaded_img/<?php echo $row['image']; ?>" width="320" height="240" alt=""></td>
            <td><?php echo $row['name']; ?></td>
			<td><?php echo $row['description']; ?></td>
            <td>
               <a href="admin_quotes.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('are your sure you want to delete this?');"> <i class="fas fa-trash"></i> delete </a>
               <a href="admin_quotes.php?edit=<?php echo $row['id']; ?>" class="option-btn"> <i class="fas fa-edit"></i> update </a>
            </td>
         </tr>
...
<section class="edit-form-container">
   ...
   if(isset($_GET['edit'])){
      $edit_id = $_GET['edit'];
      $edit_query = mysqli_query($connect, "SELECT * FROM `library_quotes` WHERE id = $edit_id");
      if(mysqli_num_rows($edit_query) > 0){
         while($fetch_edit = mysqli_fetch_assoc($edit_query)){
   ?>

   <form action="" method="post" enctype="multipart/form-data">
      <img src="uploaded_img/<?php echo $fetch_edit['image']; ?>" width="320" height="240" alt="">
      ...
      <input type="submit" value="update the library" name="update_library-quotes" class="btn">
